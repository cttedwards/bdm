...
# fit model
mdl <- fit(mdl,dat,iter=5000,run='run01')
@
...
traceplot(mdl,par=c('r','logK'),inc_warmup=FALSE)
@
<<diaghist,results="hide",fig.cap='Posterior histograms for $r$ and $ln(K)$'>>=
histplot(mdl,par=c('r','logK'))
@
<<diaghist_catchability,results="hide",fig.cap='Posterior histograms for catchability $q$'>>=
histplot(mdl,par=c('q'))
@
...
<<projplots,echo=FALSE,results="hide",fig.cap='Simple projection box-plots, illustrating predicted dynamics under different harvest rate scenarios.'>>=
par(mfrow=c(3,1))
boxplot(mdl.projections$biomass[,,1]/1e6,outline=F,main=paste('Harvest rate:',harvest.scenarios[1]),ylab='Million tonnes')
abline(v=mdl@data$T,col=4,lty=2)
boxplot(mdl.projections$biomass[,,2]/1e6,outline=F,main=paste('Harvest rate:',harvest.scenarios[2]),ylab='Million tonnes')
abline(v=mdl@data$T,col=4,lty=2)
boxplot(mdl.projections$biomass[,,3]/1e6,outline=F,main=paste('Harvest rate:',harvest.scenarios[3]),ylab='Million tonnes')
abline(v=mdl@data$T,col=4,lty=2)
@
