...
This currently represents a standard application, but in some situations it may be necessary to modify the model itself. For example, we might want to create a model that includes an observation error for every year. This is illustrated here using the same Orange Roughy example.

## Uploading a new model

The current (default) model in `bdm` can be accessed simply:
...
This new formulation includes a revision of our assumptions regarding the observation error, allowing it to change over time. To update the current model, it is necessary to re-declare the model object:

```{r, results='hide'}
mdl <- bdm(model.code = new_bdm_code)
mdl <- update_bdm(mdl,r, compile = TRUE)
```
...
This new model requries that the observation error be specified as a matrix in the input data. We first create a matrix of observation error values, with dimensions equal the index data dimensions, and then assign this matrix to the data object:

```{r, results = 'hide'}
new_sigmao <- array(0.2, dim = dim(dat$index))
sigmao(dat) <- new_sigmao
```

Finally, if a new model is loaded into the `bdm` object, it is no-longer the default model: 

```{r} 
mdl@default_model 
```

This means that some of the standard functions will not work, one of which is the initialisation function, that provides initial, sensible values for the MCMC algorithm. If the new model were to be run without an initialisation function, it will produce an error. This is a conservative, error checking mechanism. In the current context however, because the estimated parameters are the same as the default model, we can re-establish this functionality through re-setting the logical value contained in `mdl@default_model`. 

```{r, results='hide'}
mdl@default_model <- TRUE
mdl <- fit(mdl,dat)
```
```{r neworhtrace, fig.cap='Traceplots for chatham rise orange roughy fit using the new non-default model'}
traceplot(mdl,par=c('r','logK'))
```
