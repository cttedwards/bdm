...
test_that('sigmao(dat) initialise', {
    # check dimensions
    dat <- edat(index = runif(10), harvest = 1:10)
    expect_equal(dim(sigmao(dat)), c(10, 1))
    dat <- edat(index = matrix(runif(20), 10, 2), harvest = 1:10)
    expect_equal(dim(sigmao(dat)), c(10, 2))
    # check values
    sigmao.in <- matrix(runif(20), 10, 2)
    dat <- edat(index = matrix(runif(20), 10, 2), harvest = 1:10, sigmao = sigmao.in)
    expect_equal(dim(sigmao(dat)), c(10, 2))
    ...
    sigmao.in <- runif(1)
    dat <- edat(index = runif(10), harvest = 1:10, sigmao = sigmao.in)
    expect_equal(dim(sigmao(dat)), c(10, 1))
